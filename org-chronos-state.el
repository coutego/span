...
(defun org-chronos-state-next-row (state)
  ...
  (let* ((vm (org-chronos-state-view-model state))
    ...
    (if (not current-id)
        ...
        (when intervals
          (let ((payload (org-chronos-interval-payload (car intervals))))
            (setq new-id (plist-get payload :chronos-id))))
      ;; Find next
      (let ((found nil)
            (next nil))
...
(defun org-chronos-state-prev-row (state)
  ...
  (let* ((vm (org-chronos-state-view-model state))
    ...
    (if (not current-id)
        ...
        (when intervals
          (let ((payload (org-chronos-interval-payload (car (last intervals)))))
            (setq new-id (plist-get payload :chronos-id))))
      ;; Find prev
      (let ((prev nil)
            (target nil))
